...
from python_modules.file_ops import get_files_from_dir_and_its_sub_dir, get_diff_btwn_two_files, compare_two_files, \
    write_file_data, read_json_file_data
...
def compare_two_folders(first_folder_path, second_folder_path, is_email_required, excluded_sub_dirs):
    ...
    for a_first_folder_file_path in all_file_paths_in_first_folder:
        ...
        if is_diff_exist != -1 and is_diff_exist == 1:
            ...
            file_path_to_write_diff_data = "{0}/diff/{1}.html".format(compare_script_path,
                                                                      os.path.basename(a_first_folder_file_path))
    ...
    if is_email_required is True:
        ...
        user_name = "rigel"
        inner_json_config = read_json_file_data("/home/ganesh/gitrepos/program_catalog/compare/helper.json")
        for an_email in inner_json_config["emaildetails"]:
            if an_email["user_name"] == user_name:
                email_credentials = {"user_id": an_email["user_id"], "user_passwd": an_email["user_passwd"]}
                from_email = email_credentials["user_id"]
                to_email = email_credentials["user_id"]
                subject = "Dot File Comparisons."
                content = "".join(result)
